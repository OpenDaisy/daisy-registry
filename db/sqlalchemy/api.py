...
def _according_to_cidr_distribution_ip(values,network,host_interfaces_values,session):
    cluster_id=values['cluster']
    if cluster_id and network.get('assigned_networks',None):
        ...
        host_ip=""
        available_ip_list=[]
        for network_name in network['assigned_networks']:
                sql_network_id="select networks.id from networks where networks.name='"+network_name+"' and networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
                query_network_id = session.execute(sql_network_id).fetchone()
                if query_network_id is None:
                    msg = "Error:The %s is not exist!"%network_name
                    raise exception.Forbidden(msg)
                network_id_tmp=query_network_id.values()
                network_id=network_id_tmp.pop()
                ...
                available_ip_str=','.join(available_ip_list)
                sql_ip_ranges="select ip_ranges.start,end from ip_ranges where network_id='"+network_id +"'"
                query_ip_ranges = session.execute(sql_ip_ranges).fetchone()
                if query_ip_ranges:
                    ip_ranges_tmp=query_ip_ranges.values()
                    ip_ranges_end=ip_ranges_tmp.pop()
                    ip_ranges_start=ip_ranges_tmp.pop()
                    inter_num=ip_inter(ip_ranges_start)
                    ip_ranges_end_inter=ip_inter(ip_ranges_end)
                    while True:
                        inter_tmp=inter_num
                        ip_tmp=inter_ip(inter_tmp)
                        str_ip_flag=available_ip_str.find(ip_tmp)
                        if str_ip_flag==-1:
                            if inter_tmp > ip_ranges_end_inter:
                                msg = "Error:The IP address assigned by IP ranges is already insufficient."
                                raise exception.Forbidden(msg)
                            else:
                                if network_name=="MANAGEMENT":
                                    host_ip=ip_tmp
                                if network_name not in ['PRIVATE','EXTERNAL']:
                                    network_ip_dict={network_name:ip_tmp}
                                else:
                                    network_ip_dict={network_name:""}
                                str_ip=str(network_ip_dict)
                                network_ip_dict_rmp=str_ip.strip('{}')
                                network_ip_dict_rmp=network_ip_dict_rmp.replace("'","").replace(r'u',"")
                                network_ip_delete_blank=''.join([x for x in network_ip_dict_rmp if x != " "])  
                                network_ip_list.append(network_ip_delete_blank)
                                break
                        else:
                            inter_num=inter_tmp+1
                else:
                    sql_cidr="select networks.cidr from networks where networks.name='"+network_name +"' and networks.cluster_id='"+cluster_id +"'"
                    if sql_cidr is None:
                        msg = "Error:The CIDR is blank of %s!"%network_name
                        raise exception.Forbidden(msg)
                    ...
                    inter_num=ip_inter(ip_tmp_cidr)
                    while True:
                        inter_tmp=inter_num+1
                        ip_tmp=inter_ip(inter_tmp)
                        str_ip_flag=available_ip_str.find(ip_tmp)
                        if str_ip_flag==-1:
                            ip_flag=_is_in_network_range(ip_tmp, network_cidr)
                            if ip_flag:
                                if network_name=="MANAGEMENT":
                                    host_ip=ip_tmp
                                if network_name not in ['PRIVATE','EXTERNAL']:
                                ...
                                msg = "Error:The IP address assigned by CIDR is already insufficient."
                                raise exception.Forbidden(msg)
                        else:
                            inter_num=inter_tmp

        host_interfaces_values['ip']=','.join(network_ip_list)
        if not values.get('name',None):
            name_tmp=host_ip.replace('.','_')
            values['name']="host_"+name_tmp
#add by 10166727--------end-------------
...
def _host_update(context, values, host_id):
    ...
    with session.begin():
        ...
        if host_id:
            if values.has_key("os_version") and utils.is_uuid_like(values['os_version']):
                host_ref.os_version_id = values['os_version']
            elif(values.has_key("os_version") and not utils.is_uuid_like(values['os_version'])):    
                ...
                for network in orig_keys:
                    ...
                    if network.has_key('slaves'):
                        if len(network['slaves']) == 1:
                            host_interfaces_values['slave1'] = network['slaves'][0]
                        elif len(network['slaves']) == 2:
                            host_interfaces_values['slave1'] = network['slaves'][0]
                            host_interfaces_values['slave2'] = network['slaves'][1]
                        del host_interfaces_values['slaves']
                    #change by 10166727--------start-------------
                    if host_interfaces_values.has_key('ip') and host_interfaces_values['ip'] != [u''] and host_interfaces_values['ip'] !='':
                        if type(host_interfaces_values['ip'])==type([]):
                            host_interfaces_values['ip']=','.join(host_interfaces_values['ip'])
                        find_flag=host_interfaces_values['ip'].find(':')
                        if find_flag<0:
                            interface_ip=host_interfaces_values['ip']
                            ip_flag=_ip_checker_re(interface_ip)
                            if not ip_flag:
                                msg = "Error:The %s is not the right ip!"%interface_ip
                                LOG.error(msg)
                                raise exception.Forbidden(msg)
                            host_interfaces_values['ip']=host_interfaces_values['ip'].split(",")
                            network_ip_list=[]
                            if network.get('assigned_networks',None):
                                for network_assigned in network['assigned_networks']:
                                    network_ip_dict={network_assigned:interface_ip}
                                    str_ip=str(network_ip_dict)
                                    network_ip_dict_rmp=str_ip.strip('{}')
                                    network_ip_dict_rmp=network_ip_dict_rmp.replace("'","").replace(r'u',"")
                                    network_ip_delete_blank=''.join([x for x in network_ip_dict_rmp if x != " "])
                            ...
                            ip_list_tmp=host_interfaces_values['ip'].split(",")
                            for netwrok_ip in ip_list_tmp:
                                ip_flag=_ip_checker_re(netwrok_ip.split(':')[1])
                                if not ip_flag:
                                    msg = "Error:The %s is not the right ip!"%netwrok_ip.split(':')[1]
                                    raise exception.Forbidden(msg)
                    #change by 10166727--------end-------------
                    
                    #change by 10191189--------start-------------
                    if values.has_key('cluster') and not host_interfaces_values.has_key('ip'):
                        if host_interfaces_values['ip'] == [u''] or host_interfaces_values['ip'] == '':
                            _according_to_cidr_distribution_ip(values,network,host_interfaces_values,session)
                    ...
                    if host_interfaces_values.has_key('is_deployment'):
                        if host_interfaces_values['is_deployment']=="True" or host_interfaces_values['is_deployment'] == True:
                            host_interfaces_values['is_deployment']=1
                        else:
                            host_interfaces_values['is_deployment']=0
                    if host_interfaces_values.has_key('id'): del host_interfaces_values['id']
                    ...
                    if network.has_key('assigned_networks'):
                        if values.has_key('cluster'):
                            network_ip_list=host_interfaces_values['ip'].split(",")
                            network_list=network_get_all(context,values['cluster'])
                            for network_info in network_list:
                                for network_name in network['assigned_networks']:
                                    if network_name==network_info['name']:
                                        host_interfaces_values['ip'].index(network_name)
                                        for netwrok_tmp in network_ip_list:
                                            if str(network_name)==str(netwrok_tmp.split(':')[0]):
                                                assigned_networks_ref = models.AssignedNetworks()
                                                assigned_network['network_id'] = network_info['id']
                                                assigned_network['mac'] = network['mac']
                                                assigned_network['ip']=netwrok_tmp.split(':')[1]
...
def get_host_interface(context, host_id, mac=None, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        if mac is None:
           query = session.query(models.HostInterface).filter_by(host_id=host_id).filter_by(deleted=False)
        else:
           query = session.query(models.HostInterface).filter_by(host_id=host_id).filter_by(mac=mac).filter_by(deleted=False)
        ...
        for interface in host_interface:
            list = []
            assignnetwork_query = session.query(models.AssignedNetworks).filter_by(interface_id=interface.id).filter_by(deleted=False)
            assignnetwork_list = assignnetwork_query.all()
            for assignnetwork in assignnetwork_list:
                query_network_name = session.query(models.Network).filter_by(id=assignnetwork.network_id).filter_by(deleted=False).first()
                if query_network_name:
...
def get_host_interface_mac(context, mac, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        query = session.query(models.HostInterface).filter_by(mac=mac).filter_by(deleted=False)
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
        ...
        for interface in host_interface:
            ...
            assignnetwork_list = assignnetwork_query.all()
            for assignnetwork in assignnetwork_list:
                query_network_name = session.query(models.Network).filter_by(id=assignnetwork.network_id).filter_by(deleted=False).one()
                list.append(query_network_name.name)
            interface.assigned_networks = list
...
def delete_host_role(context, host_id, session=None):
    session = session or get_session()
    try:
        query = session.query(models.HostRole).filter_by(host_id=host_id).filter_by(deleted=False)
...
def delete_host_cluster(context, host_id , session=None):
    session = session or get_session()
    try:
        query = session.query(models.ClusterHost).filter_by(host_id=host_id).filter_by(deleted=False)
...
def _get_assigned_networks_by_network_id(context, network_id, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        query = session.query(models.AssignedNetworks).filter_by(network_id=network_id).filter_by(deleted=False)
...
def get_assigned_networks(context, interface_id, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        query = session.query(models.AssignedNetworks).filter_by(interface_id=interface_id).filter_by(deleted=False)
        assigned_networks = query.all()
    except sa_orm.exc.NoResultFound:
        msg = "No host found with ID %s" % host_id
        LOG.debug(msg)
...
def delete_assigned_networks(context, interface_id, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        query = session.query(models.AssignedNetworks).filter_by(interface_id=interface_id).filter_by(deleted=False)
        assigned_networks = query.all()
...
def get_os_version(context, version_id, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
...
def delete_cluster_host(context, cluster_id, session=None):
    session = session or get_session()
    try:
        query = session.query(models.ClusterHost).filter_by(cluster_id=cluster_id).filter_by(deleted=False)
...
def _cluster_update(context, values, cluster_id):
    ...
    :param context: Request context
    :param values: A dict of attributes to set
    :param cluster_id: If None, create the project, otherwise, find and update it
    """
        ...
        if cluster_id:
            ...
            if values.has_key('networks'):
                for interface_id in eval(values['networks']):                    
                    query = session.query(models.Network).filter_by(id=interface_id)
                    interfaces_values['cluster_id'] = project_ref.id
                    interfaces_values['updated_at'] = timeutils.utcnow()
                    updated = query.update(interfaces_values, synchronize_session='fetch')
            ...
            if values.has_key('logic_networks'):
                ...
                if not query:
                    raise exception.NotFound("Cluster not found,id=%s" % cluster_id)
                ...
                if logic_networks:
                    _cluster_add_logic_network(
                        ...
                        session = session,
                        status = "update")
                #---start--delete all logic_networks if values['logic_networks'] == []---
                else:
            ...
            for cluster in session.query(models.Cluster).filter_by(deleted=False).all():
            ...
            network_query = session.query(models.Network).filter_by(type="template").filter_by(deleted=False).all()
            ...
            role_query = session.query(models.Role).filter_by(type="template",cluster_id=None).filter_by(deleted=False).all()
            for sub_role_query in role_query:
                ...
                role_ref.deployment_backend = "tecs"
                ...
                service_role_query = session.query(models.ServiceRole).filter_by(role_id=sub_role_query.id).filter_by(deleted=False).all()
                for sub_service_role_query in service_role_query:
                    service_role_ref = models.ServiceRole()
                    service_role_ref.role_id = role_ref.id
                    service_role_ref.service_id = sub_service_role_query.service_id
...
def _cluster_add_routers(**params):
    ...
    if 0 == cmp(params['status'], "update"):
        router_ref = \
            session.query(models.Router).filter_by(cluster_id = params['cluster_id'])
        if router_ref.all():
            router_ref.update(
                {"deleted" : True, "deleted_at" : timeutils.utcnow()}
            )
    ...
    for router in params['routers']:
        ...
        if external_name:
            logic_network_query = \
                session.query(models.LogicNetwork).filter_by(name = external_name).filter_by(deleted=False).first()
            if logic_network_query:
                router_values['external_logic_network'] = external_name

        _update_values(router_ref, router_values)
        router_ref.save(session) # submit logic_network info to affair

        for internal_subnet_name in router.get('subnets', None):
            for logic_netwrok in logic_networks_query_all:
                subnet_query = \
                    session.query(models.Subnet).filter_by(name = internal_subnet_name, 
                                                        deleted=False, 
                                                        logic_network_id = logic_netwrok.id)
                if subnet_query.first():
                    subnet_query.update(
                        {"router_id" : router_ref.id, "updated_at" : timeutils.utcnow()}
                    )
...
def _cluster_add_logic_network(**params):
    ...
    for logic_network in params['logic_networks']:
        ...
        logic_network_values['segmentation_type'] = logic_network.get('segmentation_type', None)
        logic_network_values['segmentation_id'] = logic_network.get('segmentation_id', None)
        ...
        if  logic_network.get('physnet_name', None):
            query_list = session.query(models.Network).\
                filter_by(cluster_id = params['cluster_id']).filter_by(deleted=False).all()
            if (query_list and [valid_physnet
                               for valid_physnet in query_list
                               if logic_network['physnet_name'] == valid_physnet.name]) or \
                    logic_network.get('segmentation_type', None) == "flat":
                logic_network_values['physnet_name'] = logic_network['physnet_name']
        ...
        logic_network_ref.save(session) # submit logic_network info to affair
...
def _cluster_add_dns_nameservers(**params):
    ...
    if params['values']:
        for dns_nameservers in params['values']:
            ...
            session.query(models.DnsNameservers).filter_by(subnet_id = params['subnet_id']).filter_by(deleted=False)
...
def _check_component_id(component_id):
    ...
    now, we only check its length. The original purpose of this method is
    wrapping the different behaviors between MySql and DB2 when the component id
    length is longer than the defined length in database model.
    :param image_id: The id of the component we want to check
    :return: Raise NoFound exception if given component id is invalid
    """
    if (component_id and
       len(component_id) > models.Component.id.property.columns[0].type.length):
        raise exception.NotFound()
...
def _component_update(context, values, component_id):
    ...
    :param context: Request context
    :param values: A dict of attributes to set
    :param component_id: If None, create the component, otherwise, find and update it
    """
    ...
    with session.begin():
        ...
        if component_id:
            query = session.query(models.Component).filter_by(id=component_id).filter_by(deleted=False)

            # Validate fields for components table. This is similar to what is done
            # for the query result update except that we need to do it prior
            # in this case.
            # TODO(dosaboy): replace this with a dict comprehension once py26
            #                support is deprecated.
...
def _cluster_get(context, cluster_id, session=None, force_show_deleted=False):
    ...
    try:
        query = session.query(models.Cluster).filter_by(id=cluster_id).filter_by(deleted=False)
        # filter out deleted images if context disallows it
...
def get_logic_network(context, cluster_id, session=None, force_show_deleted=False):
    """Get an logic network or raise if it does not exist."""
    session = session or get_session()
    try:
        query = session.query(models.LogicNetwork).filter_by(cluster_id=cluster_id).filter_by(deleted=False)
        # filter out deleted images if context disallows it
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
...
def _get_subnet(context, logic_network_id=None, router_id=None, session=None, force_show_deleted=False):
    """Get an subnet or raise if it does not exist."""
    session = session or get_session()
    try:
        if logic_network_id and router_id is None:
            query = session.query(models.Subnet).filter_by(logic_network_id=logic_network_id).filter_by(deleted=False)
        ...
        msg = "No Float Ip Range found with cluster ID %s" % cluster_id
        LOG.debug(msg)
        raise exception.NotFound(msg)
    ...
    for subnet in subnets:
        ...
        subnet['dns_nameservers'] = \
            [dns_server['dns'] for dns_server in dns_nameservers] if dns_nameservers else []
        ...
        if float_ip_range and len(float_ip_range) > 1:
            int_ip_range = [[ip_into_int(float_ip[0]), ip_into_int(float_ip[1])] for float_ip in float_ip_range]
            int_ip_range = sorted(int_ip_range, key=lambda x : x[0])
            float_ip_range = [[int_to_ip(int_ip[0]), int_to_ip(int_ip[1])] for int_ip in int_ip_range]
        subnet['floating_ranges'] = float_ip_range if float_ip_range else []

    return subnets

def _float_ip_range_get(context, subnet_id, session=None, force_show_deleted=False):
    """Get an project or raise if it does not exist."""
    session = session or get_session()

    try:
        query = session.query(models.FloatIpRange).filter_by(subnet_id=subnet_id).filter_by(deleted=False)
        # filter out deleted images if context disallows it
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
...
def _dns_nameservers_get(context, subnet_id, session=None, force_show_deleted=False):
    """Get an nameservers or raise if it does not exist."""
    session = session or get_session()

    try:
        query = session.query(models.DnsNameservers).filter_by(subnet_id=subnet_id).filter_by(deleted=False)
        # filter out deleted images if context disallows it
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
...
def router_get(context, cluster_id, session=None, force_show_deleted=False):
    """Get an routers or raise if it does not exist."""
    session = session or get_session()
    try:
        query = session.query(models.Router).filter_by(cluster_id=cluster_id).filter_by(deleted=False)
        if not force_show_deleted:
            query = query.filter_by(deleted=False)
...
def cluster_get(context, cluster_id, session=None, force_show_deleted=False):
...
def get_cluster_host(context, cluster_id, session=None, force_show_deleted=False):   
    _check_cluster_id(cluster_id)
    session = session or get_session()
    try:
        query = session.query(models.ClusterHost).filter_by(cluster_id = cluster_id, deleted=False)
        # filter out deleted images if context disallows it
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
...
def _component_get(context, component_id, session=None, force_show_deleted=False):
...
def component_get(context, component_id, session=None, force_show_deleted=False):
...
def _component_services_get(context, component_id, session=None):
    ...
    session = session or get_session()
    try:
        query = session.query(models.Service).filter_by(component_id=component_id).filter_by(deleted=False)
        services = query.all()
    except sa_orm.exc.NoResultFound:
...
def _services_used_in_cluster(context, services_id, session=None):
    ...
    for service_id in services_id:
        ...
        try:
            query = session.query(models.ServiceRole).filter_by(service_id=service_id).filter_by(deleted=False)
            service_roles = query.all()
            for service_role in service_roles:
                role_ref = _role_get(context, service_role.role_id, session=session)
...
def component_destroy(context, component_id):
    ...
    if services_used:
        msg = "Services '%s' of component '%s' is using in cluster" % (','.join(services_used),component_id)
...
def _service_update(context, values, service_id):
    ...
    with session.begin():
        ...
        if service_id:
            ...
            # Validate fields for services table. This is similar to what is done
            # for the query result update except that we need to do it prior
            # in this case.
            # TODO(dosaboy): replace this with a dict comprehension once py26
            #                support is deprecated.
...
def service_get_all(context, filters=None, marker=None, limit=None,
    ...
    if marker is not None:
        marker_service = _service_get(context,
                                  marker,
...
def _role_host_member_get(context, session, member_id=None, host_id=None):
    ...
    if host_id is not None and member_id is not None:
        query = query.filter(models.HostRole.role_id == member_id).filter(models.HostRole.host_id == host_id).filter(models.HostRole.deleted == 0)
    elif member_id is not None and host_id is None:
        query = query.filter(models.HostRole.role_id == member_id).filter(models.HostRole.deleted == 0)
    elif host_id is not None and member_id is None:
        query = query.filter(models.HostRole.host_id == host_id).filter(models.HostRole.deleted == 0)
    return query.all()
...
def _role_service_member_get(context, session, member_id):
    ...
    query = query.filter(models.ServiceRole.role_id == member_id).filter(models.ServiceRole.deleted == 0)
    ...
    with session.begin():
        ...
        if role_id:
            ...
            if values.has_key('nodes'):
                ...
                if nodes_ref:
                    for host_id in orig_hosts:
                        ...
                        if host_flag==0:  # host without this role, add role to this host, should add host to cluster at the same time 
                            ...
                            if values.has_key('cluster_id'):
                                ...
                                role_def_tmp = session.query(models.Role).filter_by(id=role_id, deleted=False).one()
                                if role_def_tmp:
                                    cluster_id = role_def_tmp.cluster_id
                            if cluster_id:
                                cluster_hosts_id = get_cluster_host(context, cluster_id, session=None, force_show_deleted=False)
            ...
            if values.has_key('services'):
                ...
                services_ref = _role_service_member_get(context, session, role_id)
        ...
        if role_id:
            query = session.query(models.Role).filter_by(id=role_id).filter_by(deleted=False)
            ...
            if values.has_key('nodes'):
                ...
                if values.has_key('cluster_id') and values['cluster_id']:
                    ...
                    for host_id in orig_hosts:                    
                        ...
                        cluster_hosts_id = get_cluster_host(context, cluster_id, session=None, force_show_deleted=False)
                        if host_id not in cluster_hosts_id:  # add new record in cluster_host
                            cluster_host_ref = models.ClusterHost()
                            host_cluster_values['updated_at'] = timeutils.utcnow()
                            host_cluster_values['host_id'] = host_id
                            host_cluster_values['cluster_id'] = cluster_id
...
def host_get_all(context, filters=None, marker=None, limit=None,
    ...
    if 'status' in filters and 'cluster_id' not in filters:
        ...
        sql = "select hosts.*,cluster_hosts.cluster_id as cluster_id," \
            ...
            "roles.messages as role_messages from cluster_hosts ,hosts,roles,host_roles \
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
@utils.no_4byte_params
def _config_set_update(context, values, config_set_id):
    """
...
def config_set_add(context, values):
    ...
    return _config_set_update(context, values, None)
...
def config_set_destroy(context, config_set_id):
    ...
    with session.begin():
        ...
        config_item_refs = _config_item_get_by_config_set_id(context, config_set_id, session=session)
...
def network_get_all(context, cluster_id=None, filters=None, marker=None, limit=None,
    ...
    marker_network = None
    if marker is not None:
        marker_network = _network_get(context,cluster_id,
                                  marker,
                                  force_show_deleted=showing_deleted)
    ...
    if 0 == cmp(cluster_id, "template"):
        ...
        query = session.query(models.Network).filter_by(cluster_id=cluster_id).filter_by(deleted=showing_deleted)
    else:    
        query = session.query(models.Network).filter_by(deleted=showing_deleted)
...
def _network_get(context, network_id=None, cluster_id=None, session=None, force_show_deleted=False):
    ...
    try:
        ...
        if cluster_id is not None:
            ...
            networks = []
            for network_info in network:
                ip_range_list=[]
                ip_ranges=get_network_ip_range(context,  network_info['id'])
                if ip_ranges:
                    for ip_range in ip_ranges:
                        ip_range_dict={}
                        ip_range_dict['start']=str(ip_range['start'])
                        ip_range_dict['end']=str(ip_range['end'])
                        ip_range_list.append(ip_range_dict)
                network_info['ip_ranges']=ip_range_list
...
def update_config(session,config_flag,config_set_id,query_set_item_list,config_interface_info):
    for config_set_item in query_set_item_list.all():
        ...
        if query_config_file_info.one().name == config_interface_info['file-name']\
            ...
            del config_interface_info['file-name']
            config_interface_info['config_version']=query_config_info.one().config_version+1
            config_updated = query_config_info.one().update(config_interface_info)
            config_flag=1
            return config_flag
        else:
            continue
...
def add_config(session,config_interface_info,config_set_id,config_file_id):
    ...
    del config_interface_info['file-name']
    config_interface_info['config_file_id']=config_file_id
    config_interface_info['config_version']=1
    config_interface_info['running_version']=0
    add_config.update(config_interface_info) 
    _update_values(add_config,config_interface_info)
...
def config_interface(context, config_interface):
    ...
    with session.begin():
        if config_interface.get('role',None) and config_interface.get('cluster',None):
            query_role_info=session.query(models.Role).filter_by(name=config_interface['role']).filter_by(cluster_id=config_interface['cluster']).filter_by(deleted=False)
            if query_role_info.one().config_set_id:
                config_set_id=query_role_info.one().config_set_id
            else:
                msg = "No config_set_id found with Role Name %s" % config_interface.role-name
                LOG.error(msg)
                raise exception.NotFound(msg)
        else:
            if config_interface.get('config_set',None):
                config_set_id=config_interface.get('config_set',None)
            else:
                msg = "no role name and cluster id or config_set_id"
                LOG.error(msg)
                raise exception.NotFound(msg)
        
        try:
            for config_interface_info in eval(config_interface['config']):
